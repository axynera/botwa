...
function css() {
  return `*{box-sizing:border-box}html{color-scheme:dark}body{margin:0;min-height:100vh;background:radial-gradient(circle at 15% 10%,#123b35 0,#081413 34%,#05090a 100%);color:#eafff8;font:14px Inter,ui-sans-serif,system-ui,-apple-system,sans-serif;padding:18px}a{color:#76f7d1;text-decoration:none;font-weight:650}nav{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:22px;padding:8px;background:#0a1716aa;border:1px solid #21413d;border-radius:18px;backdrop-filter:blur(14px);box-shadow:8px 8px 24px #0008,inset 1px 1px 0 #ffffff0a}nav a{padding:9px 13px;border-radius:12px}nav a:hover{background:#16302c;color:#a7ffe6}.wrap{max-width:1280px;margin:auto}.panel{background:linear-gradient(145deg,#101f1e,#091313);border:1px solid #254542;border-radius:22px;padding:18px;box-shadow:12px 14px 35px #0009,-4px -4px 14px #18302c55,inset 1px 1px 0 #ffffff08}.top{display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-bottom:14px}input,textarea,button{font:inherit}input,textarea{background:#07100f;color:#edfff9;border:1px solid #2a4d48;border-radius:14px;padding:11px;box-shadow:inset 4px 4px 10px #0008,inset -2px -2px 5px #19312d}input:focus,textarea:focus{outline:2px solid #54e8c0aa;border-color:#64f2cb}button,.btn{border:1px solid #54dcb9;border-radius:14px;padding:10px 14px;font-weight:750;cursor:pointer;background:linear-gradient(145deg,#31dcb4,#18a98b);color:#03110d;box-shadow:5px 5px 12px #0008,-2px -2px 8px #63f5d633;transition:.18s transform,.18s filter}button:hover,.btn:hover{transform:translateY(-1px);filter:brightness(1.08)}button:active,.btn:active{transform:translateY(1px)}.secondary{background:linear-gradient(145deg,#183b37,#102a27);border-color:#315d56;color:#dffdf5}.danger{background:linear-gradient(145deg,#7a3040,#4c1e2a);border-color:#9b4b5d;color:#ffe8ed}.muted{color:#8eaaa5}.msg{min-height:22px;margin:9px 0}.ok{color:#79f5c8}.bad{color:#ff9da9}.list button{display:block;width:100%;text-align:left;margin:7px 0;background:#0d201e;color:#eafff8;border-color:#234741}.list button:hover{background:#14332e}.glow{color:#78f7d2;text-shadow:0 0 18px #42e9bd66}@media(max-width:600px){body{padding:10px}.panel{padding:14px;border-radius:18px}nav{border-radius:14px}}`;
}
...
upload.addEventListener('change',()=>{const f=upload.files[0];fileHint.textContent=f?f.name:'Pilih .js atau .mjs';if(f)uploadFile()});
async function uploadFile(){const f=upload.files[0];if(!f)return setMsg('Pilih file .js/.mjs dulu.',true);if(!/\\.(js|mjs)$/i.test(f.name))return setMsg('File harus .js atau .mjs.',true);try{const content=await f.text();const d=await api('/api/plugins',{method:'POST',body:JSON.stringify({name:f.name,content})});name.value=d.name;code.value=content;hidePreview();setMsg('Upload berhasil ✓ '+d.name);loadList();upload.value='';fileHint.textContent='Pilih .js atau .mjs'}catch(e){setMsg(e.message,true)}}
