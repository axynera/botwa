FROM node:22-alpine

WORKDIR /app

# Baileys dapat membutuhkan git untuk dependency tertentu. Chromium/Puppeteer sudah tidak dipakai.
RUN apk add --no-cache git ca-certificates ffmpeg

COPY package.json ./
RUN npm install --omit=dev --no-audit --no-fund

COPY . ./

ENV NODE_ENV=production
ENV HOST=0.0.0.0
ENV PORT=8000
ENV WA_AUTO_READ=true
ENV WA_AI_AUTO_REPLY=true
ENV WA_SESSION_DIR=/data/axynera-wa-session
ENV WA_AUTO_DOWNLOAD_IMAGES=true
ENV WA_MEDIA_TTL_MS=86400000
ENV AXYNITY_AUTO_REACTION=true
ENV WA_AUTO_ONLINE=true
ENV WA_PRESENCE_INTERVAL_MS=60000
ENV WA_ABOUT_UPDATE_MS=60000
ENV WA_ABOUT_FORCE_REFRESH_MS=300000
ENV AXYNITY_BASE_URL=https://api.axynera.my.id
ENV AXYNITY_MODEL=Nera-Plus.5
ENV AXYNITY_DEFAULT_MODE=cepat
ENV AXYNITY_TIMEOUT_MS=120000
ENV AXYNITY_STREAM_EDIT_MS=1200
ENV AXYNITY_THINK_ANIMATION_MS=900
ENV AXYNITY_MAX_IMAGE_BYTES=8388608
ENV AXYNITY_MEMORY_TURNS=20
ENV WA_STICKER_MAX_INPUT_BYTES=12582912
ENV WA_STICKER_MAX_OUTPUT_BYTES=500000
ENV WA_STICKER_MAX_SECONDS=6
ENV APPWRITE_BUCKET_ID=botwa
ENV APPWRITE_PLUGIN_SYNC_MS=30000
ENV APPWRITE_TEMP_TTL_MS=900000
ENV WA_WEB_USERNAME=admin
ENV WA_WEB_PASSWORD=change-this-password
ENV WA_WEB_SESSION_TTL_MS=86400000
ENV WA_PLUGIN_DIR=/app/plugins

EXPOSE 8000

CMD ["npm", "start"]
